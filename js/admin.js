// --- admin.js ---
// Logic for the admin puzzle editor with daily scheduling.
// Relies on functions from puzzle-core.js

// SECURITY: Admin secret for creating daily puzzles
// This file should NOT be committed to public repositories
const ADMIN_SECRET = '2x4YDZVIb5_sIyhZ'; // TODO: Replace with your actual secret
...
let scheduledDailiesData = []; // To store the current queue for collision checks
...
const loadQueue = async () => {
  ...
  queueList.innerHTML = '<div class="queue-loading">Loading...</div>';

  try {
    scheduledDailiesData = await fetchScheduledDailies(14);
    renderQueue(scheduledDailiesData);
  } catch (error) {
    console.error('Error loading queue:', error);
    queueList.innerHTML = '<div class="queue-loading">Failed to load queue</div>';
  }
};

const renderQueue = (scheduledDailies) => {
  const queueList = document.getElementById('queue-list');
  if (!queueList) return;

  const today = new Date().toISOString().split('T')[0];

  queueList.innerHTML = scheduledDailies
    .map((item, index) => {
      const isToday = item.date === today;
      const hasScheduled = item.puzzle !== null;
      ...
      const statusText = hasScheduled ? item.puzzle.puzzleName : 'Empty';
      const statusClass = hasScheduled ? 'scheduled' : '';
      const itemClass = `queue-item${isToday ? ' today' : ''}${!hasScheduled ? ' empty' : ''}`;

      // Add canvas preview if scheduled
      const previewHtml = hasScheduled
        ? `<div class="queue-preview-container"><canvas id="queue-preview-${index}" class="queue-preview"></canvas></div>`
        : '<div class="queue-preview-container queue-preview-empty"></div>';
  ...
  // Draw previews for scheduled puzzles
  scheduledDailies.forEach((item, index) => {
    if (
      item.puzzle &&
      item.puzzle.puzzleData &&
      item.puzzle.puzzleData.solutions &&
      item.puzzle.puzzleData.solutions.length > 0
    ) {
      const canvas = document.getElementById(`queue-preview-${index}`);
      if (canvas) {
        drawQueuePreview(canvas, item.puzzle.puzzleData.solutions[0]);
      }
    }
  });
};

/** Draws a small puzzle preview for the queue */
const drawQueuePreview = (canvas, puzzlePieces) => {
  if (!puzzlePieces || puzzlePieces.length === 0) return;

  const ctx = canvas.getContext('2d');
  const dpr = window.devicePixelRatio || 1;
  const size = 40; // Fixed size for queue previews
  canvas.width = size * dpr;
  canvas.height = size * dpr;
...
const publishPuzzle = async () => {
  ...
  if (isDaily && scheduledDate) {
    const existing = scheduledDailiesData.find(
      (d) => d.date === scheduledDate && d.puzzle !== null
    );
    if (existing) {
      const confirmOverwrite = confirm(
        `A puzzle ("${existing.puzzle.puzzleName}") is already scheduled for ${scheduledDate}. Do you want to overwrite it?`
      );
      if (!confirmOverwrite) {
        confirmBtn.innerHTML = 'Publish';
        confirmBtn.disabled = false;
  ...
  try {
    ...
    await createPuzzle(
      ...
      isDaily,
      scheduledDate,
      isDaily ? ADMIN_SECRET : null // Pass admin secret only for daily puzzles
    );
    document.getElementById('loading-indicator').classList.add('hidden');
